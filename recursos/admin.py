...
class Recursos (admin.ModelAdmin):
        def codigo_barras(self, instance):
            return "<a href='/admin/recursos/codigo_barras/%s'> <i style='font-size:17px' class='fa fa-file-pdf-o' aria-hidden='true'></i>  </a>" % instance.Id_recurso

        codigo_barras.short_description = "Codigo De Barras"
        codigo_barras.allow_tags = True
        codigo_barras.is_column = True

        list_display = ['Id_recurso','tipo_de_recurso','nombre_recurso','referencia','fecha_registro','fecha_de_baja','codigo_barras']
        class Meta:
		          model = Recurso
